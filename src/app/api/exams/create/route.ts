...
import { parseStudentIdentifiers, resolveUserIds } from '~/server/api/exams/csv-helper';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const examData = {
      ...
      num_questions: parseInt(formData.get('num_questions') as string, 10),
      difficulty: formData.get('difficulty') as 'easy' | 'medium' | 'hard',
      duration: parseInt(formData.get('duration') as string, 10),
      question_time_limit: parseInt(formData.get('question_time_limit') as string, 10) || 30,
