...
export async function POST(req: NextRequest) {
  try {
    ...
    const existingSubmission = await db
      .select()
      .from(results)
      .where(and(eq(results.exam_id, exam_id), eq(results.user_id, userId)))
      .execute();
    ...
    answers.forEach((answer) => {
      ...
      if (!question) {
        console.warn(
          `Question with ID ${answer.question_id} not found in exam questions`
        );
        return;
      }
      ...
      const correctIndex = Number.parseInt(question.correct_answer, 10);
    ...
    console.log(
      `Student ${userId} scored ${score}/${totalQuestions} (${percentageScore}%) on exam ${exam_id}`
    );
