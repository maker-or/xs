...
const StagePage = ({ params }: StagePageProps) => {
  ...
  if (stageResult.error || !stageResult.stage) {
    return (
      <main className="relative min-h-[100svh] w-[100svw]">
        ...
        {/* Back button */}
        <Link
          className="absolute top-6 left-6 z-30 rounded-lg border border-white/20 bg-black/60 p-3 text-white backdrop-blur-md transition-all duration-200 hover:bg-white/10"
        ...
        <div className="relative z-20 flex h-full w-full flex-col items-center justify-center px-8">
          <div className="text-center">
            <h1 className="mb-4 font-light text-4xl text-red-400">
              Error Loading Stage
            </h1>
  ...
  return (
    <div className="relative">
      {/* Back button overlay */}
      <Link
        className="absolute top-6 left-6 z-50 rounded-lg border border-white/20 bg-black/60 p-3 text-white backdrop-blur-md transition-all duration-200 hover:bg-white/10"
        href={`/learning/learn/${courseId}`}
      >
        <ArrowLeft className="h-5 w-5" />
      </Link>
