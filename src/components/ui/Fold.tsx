...
const Fold = () => {
  ...
  const {
    ...
    error,
    mutate,
  }: SWRResponse<FolderType[], Error> = useSWR<FolderType[], Error>(
    '/api/folder',
    fetcher,
    {
  ...
  const addFolder = async () => {
    ...
    try {
      await mutate(
        async (currentFolders = []) => [...currentFolders, newFolder],
        {
  ...
  if (isLoading) {
    return (
      <div className="font-serif text-[#f7eee38c]">Preparing your notes...</div>
    );
  }

  if (error) {
    return (
      <div className="font-serif">Error fetching folders: {error.message}</div>
    );
  }
  ...
  return (
    <div className="items-left justify-left flex w-full flex-col gap-1">
      <Group />
      <div className="flex w-full items-center justify-between" id="addNew">
        <h2 className="font-serif text-white text-xl">Notebooks</h2>
        <button
          className="rounded-lg bg-orange-600 px-3 py-1 text-md text-white"
          onClick={addFolder}
        >
          +
        </button>
      ...
      {folders.length === 0 ? (
        <div className="mt-4 mb-4 flex w-full items-end justify-end">
          <p className="font-serif text-[#f7eee3]/60 text-lg">
