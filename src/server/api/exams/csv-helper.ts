...
import { UserIdentifier } from './schemas';
...
export async function parseStudentIdentifiers(csvData: string[]): Promise<UserIdentifier[]> {
  ...
  const hasHeader = potentialHeader.includes('email') || 
