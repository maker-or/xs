...
export const createPDF = (messages: Message[]): void => {
  ...
  const dateWidth =
    (doc.getStringUnitWidth(formattedDate) * 12) / doc.internal.scaleFactor;
  ...
  const renderLine = (
    ...
    for (const segment of segments) {
      const fontStyle: string = segment.styles.includes('italic')
        ? 'italic'
        : 'normal';
      ...
      for (const line of lines) {
        const lineWidth: number =
          (doc.getStringUnitWidth(line) * 12) / doc.internal.scaleFactor;
  ...
  const renderCodeBlock = (
    ...
    for (const line of codeLines) {
      if (
        (doc.getStringUnitWidth(line) * 11) / doc.internal.scaleFactor <=
        maxWidth - 30
      ) {
        lines.push(line);
      } else {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const keywords =
        /(class|function|const|let|var|if|else|for|while|return|import|from|export|default|interface|type|extends|implements|public|private|protected)\b/g;
      const strings =
        /"([^"\\]*(\\.[^"\\]*)*)"|\\'([^'\\]*(\\.[^'\\]*)*)'|`([^`\\]*(\\.[^`\\]*)*)`/g;
      ...
      if (keywords.test(line) || strings.test(line) || comments.test(line)) {
        ...
        if (segments.length > 0) {
          ...
          for (const segment of segments) {
            ...
            xPos +=
              (doc.getStringUnitWidth(segment.text) * 11) /
          ...
          doc.text(
            line,
            startX + 15 + (showLineNumbers ? lineNumberWidth : 0),
            y
          );
        }
      } else {
  ...
  const renderMathBlock = (
    ...
    for (const line of lines) {
      ...
      // Center math content
      const lineWidth =
        (doc.getStringUnitWidth(line) * 13) / doc.internal.scaleFactor;
  ...
  const renderTable = (
    ...
    for (const row of tableData) {
      for (let c = 0; c < row.length; c++) {
        const cell = row[c] || '';
        const width =
          (doc.getStringUnitWidth(cell) * 10) / doc.internal.scaleFactor;
        colWidths[c] = Math.max(colWidths[c], width);
      }
    }
    const totalWidth: number = colWidths.reduce(
    ...
    const tableWidth: number =
      colWidths.reduce((sum: number, w: number): number => sum + w, 0) +
      colCount * 8;
    ...
    doc.roundedRect(
      ...
      y - 20,
      tableWidth + 30,
      tableData.length * (LINE_HEIGHT + 10) + 40,
      10,
      10,
      'F'
    );
  ...
  const renderInlineMath = (
    ...
    for (const placeholder of mathPlaceholders) {
      ...
      if (parts[0]) {
        ...
        const textWidth =
          (doc.getStringUnitWidth(parts[0]) * 11) / doc.internal.scaleFactor;
      ...
      if (mathIndexMatch && mathIndexMatch[1]) {
        ...
        if (mathExpr) {
          ...
          const mathWidth =
            (doc.getStringUnitWidth(mathText) * 12) / doc.internal.scaleFactor;
    ...
    if (remainingText) {
      ...
      const textWidth =
        (doc.getStringUnitWidth(remainingText) * 11) / doc.internal.scaleFactor;
  ...
  const renderStyledText = (
    ...
    if (line.match(/^<<H1>>(.*)<<H1>>$/)) {
      ...
      // Add underline
      const textWidth =
        (doc.getStringUnitWidth(text) * 18) / doc.internal.scaleFactor;
    ...
    if (line.match(/^\d+\.\s/)) {
      ...
      if (match && match[1] && match[2]) {
        ...
        const numberWidth: number =
          (doc.getStringUnitWidth(number + '.') * 12) /
          doc.internal.scaleFactor;
  ...
  messages.forEach((message: Message, index: number) => {
    ...
    if (message.role === 'user') {
      ...
      const {
        ...
        tables,
        inlineCodes,
      }: FormattedContent = formatMarkdownForPDF(message.content);
      ...
      for (const part of parts) {
        if (
          (part.includes('__CODE_BLOCK_') ||
            part.includes('__MATH_BLOCK_') ||
            part.match(/^<<H1>>.*<<H1>>$/) ||
            part.match(/^<<H2>>.*<<H2>>$/)) &&
          !prevWasSpecialBlock
        ) {
        ...
        if (part.includes('__CODE_BLOCK_')) {
          ...
          if (tableIndexMatch && tableIndexMatch[1]) {
            const tableIndex = Number.parseInt(tableIndexMatch[1]);
          ...
          if (mathIndexMatch && mathIndexMatch[1]) {
            ...
            // Fix unsafe return by explicitly typing the return value
            const newY: number = renderMathBlock(
              doc,
              mathContent,
              margin,
              y,
          ...
          if (
            part.trim().startsWith('__INLINE_CODE_') &&
            part.trim().endsWith('__')
          ) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFontSize(9);
    doc.setTextColor(255, 98, 0);
    doc.text('Created with Sphereai.in', pageWidth - margin, pageHeight - 25, {
      align: 'right',
    });
  }
